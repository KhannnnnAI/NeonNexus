...
function updateUserUI() {
 ...
 console.error('Failed to parse Steam user data');
  ...
  // Priority 2: Check Guest user from dashboard session
  if (!userData) {
    try {
      const dashSession = JSON.parse(localStorage.getItem('dashboard_auth_session_v1') || 'null');
      if (dashSession && dashSession.isGuest) {
        userData = {
          displayName: dashSession.displayName || 'Guest Gamer',
          photoURL: '/Image/profile.png',
          isGuest: true
        };
 console.log(' Guest user detected:', userData.displayName);
      }
    } catch(e) {}
  }

  // Priority 3: Check Firebase Auth (Google ONLY - not anonymous)
...
async function initAccountPage() {
 ...
 console.error(' No user ID available');
  ...
  // === GUEST: Freeze membership section ===
  if (_isGuestUserId(currentUserId)) {
    const section = document.getElementById('membership-section');
    const overlay = document.getElementById('membership-guest-overlay');
    if (section && overlay) {
    ...
    // === Normal user: Load membership from Firebase ===
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // === 1. KIỂM TRA SESSION ĐĂNG NHẬP HIỆN TẠI (CHỈ NHẬN GUEST KHI THỰC SỰ LÀ GUEST) ===
  let currentSession = null;
  try {
    currentSession = JSON.parse(localStorage.getItem('dashboard_auth_session_v1') || 'null');
  } catch(e) {}

  // Nếu session hiện tại là Guest (user vừa bấm Login as Guest) -> Bỏ qua Firebase
  if (currentSession && currentSession.isGuest && currentSession.uid) {
    console.log(' Guest user session detected immediately:', currentSession.uid);
    currentUserId = currentSession.uid;
    isCurrentUserGuest = true;
    updateUserUI();
    await initAccountPage();
    return; // Dừng tại đây, không load Firebase
  }

  // === 2. CÁC TRƯỜNG HỢP CÒN LẠI (Google, Steam, hoặc Chưa đăng nhập) ===
  // Bỏ hoàn toàn việc check `guest_id` trần trụi ở đây để tránh đè lên Steam/Google
  ...
  auth.onAuthStateChanged(async (firebaseUser) => {
    ...
    if (firebaseUser) {
      // ✅ STEAM LOGIN (Anonymous Firebase Auth)
      if (firebaseUser.isAnonymous) {
        ...
        while (retries < maxRetries && !steamUserData) {
          ...
          console.log('Waiting for Steam data...');
          await new Promise(resolve => setTimeout(resolve, 200));
          retries++;
        }

        if (steamUserData) {
          currentUserId = steamUserData.steamId || steamUserData.uid || firebaseUser.uid;
          console.log('Steam user detected (anonymous), account:', currentUserId);
        } else {
          currentUserId = firebaseUser.uid;
          console.log('No Steam profile data yet, fallback uid:', currentUserId);
        }

        isCurrentUserGuest = false; // Đảm bảo clear cờ Guest
        updateUserUI();
        await initAccountPage();
        return;
      }
      ...
      // ❌ CHƯA ĐĂNG NHẬP
      console.log(' No Firebase user & No Guest session -> Login Required');
      currentUserId = null;
  ...
  window.addEventListener('user-login', (event) => {
    const sessionAccount = event.detail?.sessionAccount || '';
    const eventUserId = event.detail?.userId || '';
    const resolvedUserId = eventUserId || sessionAccount;
    
    // Cập nhật lại cờ isGuest khi có event login
    isCurrentUserGuest = !!(event.detail?.user?.isGuest);
    
    if (!resolvedUserId || resolvedUserId === currentUserId) {
      return;
    }
    console.log(' User logged in:', resolvedUserId);
    currentUserId = resolvedUserId;

    // Update UI header
    updateUserUI();
    loadUserKeys(resolvedUserId);
  });
...
async function loadUserKeys(userId) {
  ...
  if (_isGuestUserId(userId)) {
    console.log(' Loading guest keys from localStorage...');
    showLoading();
    stopKeysListener();
    
    try {
      const guestKeysJson = localStorage.getItem('guest_purchased_keys');
      const guestKeys = guestKeysJson ? JSON.parse(guestKeysJson) : [];
      console.log(' Guest keys found:', guestKeys.length);
      
      setTimeout(() => {
        renderKeys(guestKeys);
      }, 300); // Brief delay for loading UX
    } catch(e) {
      console.error('Failed to load guest keys:', e);
      showEmptyState();
    }
    return;
  }

  // === NORMAL USER: Load from Firebase ===
 ...
 console.warn('Firebase not available, using mock data');
  ...
  try {
    if (activeKeysUserId === userId && typeof unsubscribeListener === 'function') {
      return;
    }

    showLoading();
    stopKeysListener();
    activeKeysUserId = userId;

    // Load keys with real-time listener
    unsubscribeListener = await window.firebaseCart.loadPurchasedKeys(userId, (orders) => {
      if (activeKeysUserId !== userId) return;
      renderKeys(orders);
    });
  } catch (error) {
