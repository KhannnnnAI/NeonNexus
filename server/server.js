...
if (!stripeKey) {
 console.warn('WARNING: STRIPE_SECRET_KEY is missing in .env file! Stripe payments will fail.');
}
if (!SESSION_SECRET) {
 console.warn('WARNING: SESSION_SECRET is missing in .env file! Using temporary fallback secret.');
}
...
if (stripeKey && stripeKey.trim().length > 0) {
 ...
 console.warn(' STRIPE_SECRET_KEY missing. Stripe features disabled.');
}
...
// ==========================================
const { corsMiddleware, getCorsOptions, setCorsHeaders } = require('./cors-config');
...
// Serves: /api/steam/featured
// Used by final_local to get real-time sale + discount_expiration (no CORS)
// ==========================================
const steamFeaturedCache = { data: null, time: 0 };
const STEAM_FEATURED_TTL = 15 * 60 * 1000; // 15 min cache
...
app.get('/api/steam/featured', async (req, res) => {
    ...
    try {
        ...
        console.log('[Steam Featured] Served fresh from Steam');
        res.set('X-Cache', 'MISS');
        return res.json(response.data);
    } catch (error) {
        console.error('[Steam Featured] Error:', error.message);
        if (steamFeaturedCache.data) { res.set('X-Cache', 'STALE'); return res.json(steamFeaturedCache.data); }
        return res.status(500).json({ error: 'Failed to fetch Steam featured' });
    }
});
...
app.get('/api/steam/price', async (req, res) => {
 ...
 console.log(`Fetching Steam prices for ${missingIds.length} uncached games (VN + US)...`);
    ...
    // Steam API accepts around <=50 IDs per call, so we keep batches at 40.
    const BATCH_SIZE = 40;
 ...
 console.error('Steam Price API Error:', error.message);
        ...
        // Return whatever we already have from cache to keep storefront responsive.
...
app.get('/api/games/trailer', async (req, res) => {
 ...
 console.warn(' No Twitch Keys. Returning Mock Trailer.');
 console.warn(' No Twitch Keys. Returning Mock Trailer.');
 ...
 console.log(` IGDB: No trailer found for ${gameName}`);
...
app.post('/api/create-zalopay-payment', async (req, res) => {
    try {
        ...
        const embed_data = {
            ...
            redirecturl: redirectUrl || process.env.DEFAULT_RETURN_URL || 'https://nenonexus-digital-game-store.web.app/html/keygen.html'
        };
