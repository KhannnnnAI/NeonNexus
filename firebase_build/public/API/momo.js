...
const config = {
    ...
    partnerName: process.env.MOMO_PARTNER_NAME || 'NeonNexus',
    storeId: process.env.MOMO_STORE_ID || 'NeonNexusStore',
    redirectUrl: process.env.MOMO_REDIRECT_URL || process.env.DEFAULT_RETURN_URL || 'https://nenonexus-digital-game-store.web.app/keygen',
...
const isMomoConfigured = Boolean(config.accessKey && config.secretKey && config.ipnUrl);
...
function createMomoPayment(amount, orderId, orderInfo = 'Pay with MoMo', returnUrl = null, extraData = '') {
    return new Promise((resolve, reject) => {
        if (!isMomoConfigured) {
            reject(new Error('MoMo is not configured. Set MOMO_ACCESS_KEY, MOMO_SECRET_KEY and MOMO_IPN_URL.'));
            return;
        }
